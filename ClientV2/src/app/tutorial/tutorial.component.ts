	...
	fileUploads(event) {
		...
		if (this.files && this.files.length > 0) {
			if (extension != 'xlsx' && extension != 'xls' && extension != 'csv') {
				...
				this.tamano = '';
				this.disabledCargaArchivo = true;
			} else {
				...
				if (extension == 'xlsx' || extension == 'xls') {
					...
					reader.onload = (e: any) => {
						...
						if (this.duplicateColunms.length == 0) {
							this.disabledCargaArchivo = false;
						}
					};
					...
					reader.onload = (e) => {
						this.papa.parse(file, {
							complete: (result) => {
								this.duplicateColunms = result.data[0].filter(
									(item, index, array) => {
										return array.indexOf(item) !== index;
									}
								);
	...
	cargarDatos(stepper: MatStepper) {
		...
		if (this.files && this.files.length > 0) {
			...
			if (extension == 'xlsx' || extension == 'xls') {
				...
				reader.readAsBinaryString(this.fileEvent.files[0]);
			} else {
				const file: File = this.files.item(0);
				const reader: FileReader = new FileReader();
				reader.readAsText(file);
