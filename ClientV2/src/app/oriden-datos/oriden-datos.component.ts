...
export class OridenDatosComponent implements OnInit {
	...
	carrgarTablas() {
		this.uploadService.getTables(this.selectEmpresa, 1, 10).subscribe((res) => {
			...
			if(res.length > 0 ){
				...
				// Se carga la tabla si fue seleccionada de la vista anterior
				let tableId = this.globalService.getSelectTable();
				if(tableId !== undefined){
					this.abrirTabla(tableId);
				}
		});
	}
	...
	abrirTabla(id) {
		let idx = this.tablas[id].tabla_id;
		if (!this.agregadosIdx.includes(idx)) {
			this.duplicateColunms = [];
			this.tamano = 0;
			this.agregadosIdx[this.asyncTabs.length] = idx;
			this.cargarTabs(this.tablas[id].tabla_id, this.tablas[id].nombre, '', false);
